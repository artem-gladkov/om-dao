...
export const App: FC<AppProps> = observer(({className, children, ...otherProps}) => {
  const [ethereumStore] = useState(() => new EthereumStore())
  const {initialized, isCorrectNetwork, changeToAvailableNetwork} = ethereumStore
          ...
          ? <>
            {
              isCorrectNetwork
                ...
                : <div className={'appNetworkAlert'}>
                  <Container>
                    <div>
                      <h1>Для работы приложения необходима сеть {AVAILABLE_NETWORK}</h1>
                      <Button onClick={changeToAvailableNetwork}>Сменить сеть на {AVAILABLE_NETWORK}</Button>
                    </div>
                  </Container>
                </div>
            }
