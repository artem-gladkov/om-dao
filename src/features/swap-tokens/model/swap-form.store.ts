...
export class SwapFormStore {
  ...
  constructor(private _signer: JsonRpcSigner) {
    ...
    this._sourceContract = new Contract(TOKEN_ADDRESS[TOKEN_SYMBOLS.USDT], TOKEN_ABI[TOKEN_SYMBOLS.USDT], _signer)
    this._destinationContract = new Contract(TOKEN_ADDRESS[TOKEN_SYMBOLS.OMD], TOKEN_ABI[TOKEN_SYMBOLS.OMD], _signer)
  }

  public onSwap = async ({sourceAmount, destinationAmount, isRearranged}: BaseTokensFormSubmitData) => {
  ...
  private fetchAllowedAmount = async (): Promise<number> => {
    ...
    try {
      ...
      const allowanceAmount = await this._sourceContract.allowance(signerAddress, this._destinationContract.address)
  ...
  private approveSwapAmount = async (amount: string): Promise<void> => {
    ...
    try {
      ...
      const approveTransaction = await this._sourceContract.approve(this._destinationContract.address, unit256Amount)
  ...
  private buyToken = async (amount: string): Promise<void> => {
    ...
    try {
      ...
      const unit256Amount = parseUnits(amount, decimals)
      const buyTransaction = await this._destinationContract.buyToken(unit256Amount)
      this.swapStatus = SwapStatus.AWAITING_BLOCK_MINING
  ...
  sellOMD = async (amount: string) => {
    ...
    try {
      ...
      const unit256Amount = parseUnits(amount, decimals)
      const sellTransaction = await this._sourceContract.sellToken(unit256Amount)
      this.swapStatus = SwapStatus.AWAITING_BLOCK_MINING
