...
export const Wallets: FC<WalletsProps> = ({className, onClickWallet, children, ...otherProps}) => {
  return (
    <div className={classNames(styles.wallets, className)} {...otherProps}>
      {WALLETS.map(({name, ...props}) => <Wallet key={name} name={name} onClick={onClickWallet} {...props} />)}
    </div>
  )
}
